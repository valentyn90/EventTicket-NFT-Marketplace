...
import { getFileFromSupabase, getFileLinkFromSupabase, getNftById, getScreenshot } from "@/supabase/supabase-client";
import { ModalContentType } from "@/types/ModalContentType";
import Nft from "@/types/Nft";
import ListingData from "@/types/SellData";
...
const CardListItemNew: React.FC<Props> = ({
  listType,
  nft,
  serial_no = 1,
  sellData,
  ...
  // const [mktplaceNft, setMktplaceNft] = useState<Nft>();

  const MARKET_ENABLED = process.env.NEXT_PUBLIC_ENABLE_MARKETPLACE === "true";

  // useEffect(() =>{
  //   if(nft_id){
  //     getNftById(nft_id).then(res => {
  //       const d = res.data as Nft;
  //       setMktplaceNft(d);
  //     })
  //   }
  // }
  ...
  // useEffect(() => {
  //   if (screenshot_url && listType === "marketplace") {
  //     setScreenshot(screenshot_url)
  //     setLoaded(true);
  //   }
  //   else 
  //   if (nft_id && listType !== "marketplace") {
  //     getScreenshot(nft_id).then((res) => {
  //       if (res.publicUrl) {
  //         setScreenshot(res.publicUrl);
  //         setLoaded(true);
  //       }
  //       if (res.error) {
        ...
        <>
          <Box
            ...
            onClick={() => {
              ...
              //   userStore.ui.setFieldValue("selectedSN", serial_no);
              //   userStore.ui.openModalWithNft(mktplaceNft, listType);
              // }
          ...
          >
            ...
            >
              {(price && sol_price) ? `$${(price*sol_price).toFixed(0)}` : "View"}
            </Box>
            {/* {listType === "listings" && (
              <Box
                w="100%"
            ...
            {listType === "marketplace" && (
              <Box
                ...
                // disabled={price.is || !MARKET_ENABLED}
                onClick={() => {
                  if (!price || !MARKET_ENABLED) {
                    return;
                  }
...
export default observer(CardListItemNew);
