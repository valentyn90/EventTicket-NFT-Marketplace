...
const Challenges: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const getChallengeData = async () => {
      const { data: challenge_data } = await supabase.from('drop').select('*')
      ...
      <Spinner size="xl"/> :
        ...
        >
          ...
          {activeChallenges.map((challenge, index) => {
            ...
            return (
              <VStack p={2}  bg="blueBlack" borderRadius={6} border="2px" borderColor="blueBlack2" onClick={()=>{setLoading(true); router.push(`/drops/${challenge.id}`)}}>
                <StaticCard nft_id={challenge.nfts[2]} width={150} />
                <Heading size="sm">
                  {challenge.player_name}{challenge.player_name.slice(-1)=="s" ? "'" : "'s"} Drop
                </Heading>
                </VStack>
            )
          })}
